-- Enforce the customer-facing report contract at the database boundary. MCP
-- validation remains useful feedback, but authenticated callers cannot bypass
-- evidence ownership, structure, size, or natural-language requirements.
...
begin
  ...
  from (
    ...
    from jsonb_array_elements(
      case when jsonb_typeof(evidence_payload -> 'evidence') = 'array'
        then evidence_payload -> 'evidence' else '[]'::jsonb end
  ...
  if cardinality(cited_evidence_ids) = 0
     or exists (
       select 1 from unnest(cited_evidence_ids) cited_id
...
revoke all on function public.submit_company_report_analysis(uuid, uuid, text, text, jsonb)
  from public, anon, service_role;
grant execute on function public.submit_company_report_analysis(uuid, uuid, text, text, jsonb)
  to authenticated;
