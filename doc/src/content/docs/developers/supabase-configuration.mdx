...
The script will ask you for the Supabase instance name and a masked first-administrator bootstrap token, create the database, apply the migrations and deploy the edge functions. The token is sent to Supabase through a permission-restricted temporary environment file and is not written to the frontend environment. Finally, the script will create a `.env.production.local` file with your remote Supabase configuration.
